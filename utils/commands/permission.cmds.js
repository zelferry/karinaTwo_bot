...
module.exports = async({user, bot}) => {
    let botperm;
    let userperm;

    if(user.length > 0){
        let permission2 = permission1.Member({
            MemberPerm: user
        });
        
        userperm = permission2.join(", ")
    } else {
        userperm = "nenhuma permissão e necessário" 
    }

    if(bot.length > 0){
        let permission3 = permission1.Client({
            ClientPerm: bot
        });

        botperm = permission3.join(", ")
    } else {
        botperm = "nenhuma permissão e necessário"
    }
