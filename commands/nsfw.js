...
var randGay = Math.floor(Math.random() * listNsfw.length)
  
let arr = listNsfw
const embed = new Discord.MessageEmbed().setImage(listNsfw[randGay])
 ...
 let author = message.author;
  ...
  collector.on('collect',async r => {
    ...
    user.id!=client.user.id&&r.users.remove(user);
      ...
      if(r.emoji.name=== "🔁"){
randGay = Math.floor(Math.random() * listNsfw.length)
  
arr = listNsfw
yeste = randGay
const embed = new Discord.MessageEmbed().setImage(listNsfw[randGay]).setColor("#7B68EE").setFooter(`${randGay} / ${arr.length-1}`)
msg.edit(embed)
      }
      if(r.emoji.name === "❌"){
