...
const roleColor = message.guild.me.displayHexColor === "#FF1493" ? "##FF1493" : message.guild.me.displayHexColor;
   ...
   if (args[0]) {
     const comEmbed = new Discord.MessageEmbed().setColor(roleColor).setDescription("<> = obrigatório\n[] = opcional\n\n`texto` = inserir um texto\n`imagem` = anexar uma imagem\n`mumero` = um numero de 1 a 🔁\n`usuário` = ID do usuário ou a mersão dele\n`canal` = canal de texto").addField("nome do comando:",`${props.help.name}`).addField("descrição:", `${props.help.description ? props.help.description : "???"}`).addField("aliases",`${aliases ? aliases : "???"}`).addField("como usar?:",`\`${prefix}${props.help.usage ? props.help.usage : "???"}\``).addField("permissões necesarias:",`${props.help.permisoes ? props.help.permisoes : "???"}`)
